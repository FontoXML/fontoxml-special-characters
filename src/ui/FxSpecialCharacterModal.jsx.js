...
const storagePrefix =
	...
	reactToAngularModalBridge.operationData.characterSet +
	'|';

const searchInputContainerStyles = { maxWidth: '20rem', width: '100%' };
const searchInputPlaceholder = t('Search by name or codepoint');

class FxSpecialCharacterModal extends Component {
	allSymbols = specialCharactersManager
		.getCharacterSet(reactToAngularModalBridge.operationData.characterSet);
	...
	componentDidMount () {
		if (localStorageService.hasData(storagePrefix + 'storedRecentCharacters')) {
			try {
				this.recentSymbols = JSON.parse(
					localStorageService.getData(storagePrefix + 'storedRecentCharacters')
				);
				this.filterOptionsForRecentSymbols =
					createFilterOptionsFromSymbols(this.recentSymbols);
				...
				console.error('Error during JSON parsing of ' +
					'localStorageService.getData(' + storagePrefix + 'storedRecentCharacters")');
			}
		}
	}
	...
	filterSymbolsForSelectedFilterOption (symbols) {
		...
		return symbols
			.filter(symbol => symbol.labels.includes(this.state.selectedFilterOption.label));
	}
	...
	determineDisplayedSymbols () {
		...
		return this.allSymbols.filter(symbol => (
			...
			symbol.labels
				.some(label => label.toLowerCase().includes(searchInputValue.toLowerCase())) ||
			// or match on one of the code points
			symbol.codePoints
				.some(codePoint => codePoint.toLowerCase().includes(searchInputValue.toLowerCase()))
		));
	}
	...
	determineFilteredDisplayedSymbols (displayedSymbols) {
		...
		return displayedSymbols.filter(symbol => symbol.labels.includes(selectedFilterOption.label));
	}
	...
	determineFilterOptions (displayedSymbols) {
		...
		return this.filterOptionsForAllSymbols
			.filter((filterOption) => displayedSymbolsLabels.includes(filterOption.label));
	}
	...
	handleCloseButtonClick = () => reactToAngularModalBridge.closeModal();

	handleSubmitButtonClick = () => {
		const selectedSymbol = this.state.selectedSymbols[0];

		const indexInRecentSymbols = this.recentSymbols
			.findIndex(recentSymbol => recentSymbol.id === selectedSymbol.id);
		...
		localStorageService.setData(
			storagePrefix + 'storedRecentCharacters',
			JSON.stringify(this.recentSymbols)
		);

		reactToAngularModalBridge.onModalSubmit({ text: characterToString(selectedSymbol) });
	};
	...
	render () {
		...
		return (
			<Modal isFullHeight size='m'>
				<ModalHeader
					icon={ reactToAngularModalBridge.operationData.modalIcon }
						...
						<ModalContent>
							...
							<ModalContent flex='1' flexDirection='column'>
								<Flex justifyContent='center' paddingSize='m'>
									{ this.renderResultsCounter(displayedSymbols.length, searchInputValue) }
								</Flex>
...
export default FxSpecialCharacterModal;
